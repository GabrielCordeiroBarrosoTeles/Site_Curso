...
<?php
    require '../dbcon.php';
    $categorias = ["Programação", "Banco de Dados"];
    ...
    foreach ($categorias as $categoria) {
        ...
        if (mysqli_num_rows($query_run) > 0) {
                                    ...
                                    echo '<div class="modal-body">';
                                        echo '<div class="row">';
                                            echo '<div class="col-md-6">';
                                                echo '<img src="../adm/img/categoria/'.$cursos["imagem_capa"].'" alt="" class="img-fluid d-block mx-auto mb-3" style="max-width: 100%; max-height: 300px;">';
                                            echo '</div>';
                                            echo '<div class="col-md-6">';
                                                echo '<p><strong>Função:</strong> ' . $cursos["categoria"] . '</p>';
                                                echo '<p><strong>Detalhe:</strong> ' . $cursos["descricao"] . '</p>';
                                                echo '<p><strong>Professor:</strong> ' . $cursos["nome_professor"] . ' ' . $cursos["sobrenome_professor"] . '</p>';
                                                echo '<p><strong>Valor:</strong> <span style="color: #ff0000; font-size: 1.5em;">R$ ' . number_format($cursos["valor"], 2, ',', '.') . '</span></p>';
                                            echo '</div>';
                                        echo '</div>';
                                    echo '</div>';
                                    echo '<div class="modal-footer">';
                                        echo '<button type="button" class="btn btn-secondary" data-dismiss="modal">Fechar</button>';
                                        echo '<a href="inscricao.php?curso_id=' . $cursos["id"] . '" class="btn btn-primary">Inscreva-se Agora</a>';
                                    echo '</div>';
                                echo '</div>';
                            echo '</div>';
                        echo '</div>';
            ...
            echo "<h5>Nenhum produto encontrado na categoria " . $categoria . "</h5>";
        }
    }
?>
